...
const htmlPath = path.resolve('D:/myfolder/動画生成/技術解説/projects/fullstack_myth/slides/index.html');
const outputPath = path.resolve('D:/myfolder/動画生成/技術解説/projects/fullstack_myth/remotion/scenes/SlideScenes.tsx');
...
function htmlToJsx(h) {
    ...
    j = j.replace(/&#10084;/g, '❤');
    j = j.replace(/&amp;/g, '&');
    j = j.replace(/style="--w:(\d+%);?\s*background:var\(([^)]+)\)"/g,
        `style={{ '--w': '$1', background: 'var($2)' } as React.CSSProperties}`);
    j = j.replace(/style="--w:(\d+%)"/g,
        `style={{ '--w': '$1' } as React.CSSProperties}`);
    j = j.replace(/style="margin-top:\s*([^";]+);?"/g,
        `style={{ marginTop: '$1' }}`);
    j = j.replace(/style="color:\s*var\(([^)]+)\);?"/g,
        `style={{ color: 'var($1)' }}`);
