...
**文字は大きく。** スマホ視聴前提。画面に出る文字は原則「読ませる文字」で、下表のサイズで大きく描く。小さくしてよいのは「記号としての文字」——大量に並んだ図形の連番・添字のような、読ませず形として置くだけの文字——に限る。これが唯一の例外：

| 用途 | サイズ（px） |
|---|---|
| 大見出し・タイトル | 76〜92 |
| 対話字幕 本文 | 52〜58 |
| 図の装置名・主要ラベル（既定の読む文字） | 48〜58 |
| 図中の注記・補足（読む文字） | 42〜48 |
| 字幕の話者名・セクションタイトル | 40〜46 |
| 記号としての文字（連番・添字など、読ませない） | 〜34 |

**読ませる文字の下限は 42px。** これ未満は記号としての文字にのみ許す。

**`scale` で縮小する装置でも、テキストは縮小しない。** 装置を `scale={0.6}` などで小さく描くとき、内部の文字に `scale` を掛けると下限を割る。文字サイズは固定値（または下限つき）で持ち、`scale` の影響から外す。フォントサイズ定数（例：`FS_TITLE = 84` `FS_LABEL = 54` `FS_NOTE = 44` `FS_SYMBOL = 32`）をファイル冒頭に置き、各 `<text>` で使う。
...
### 5.3 テキストを内包する箱は、テキスト幅から箱幅を派生計算する

**ラベル付きカード／軸ラベル帯／トークン箱／注記など、内部にテキストを持つ箱は、固定値で幅を持たない**。固定値だと、文字数が箱の見込みを超えた瞬間に必ずはみ出す（例: 幅 300px のサブカードに「自己回帰モデル」7字×48px≒352px が入りきらない）。

design_overall が「最大文字数 M 字、フォント基準 N px、余白 padX」を指定している前提で、箱の幅をテキストから派生：

```tsx
// CJK は概ね 1em/字（半角混じりはやや短い）。1.05 は微余裕
const measureLabelWidth = (text: string, fontSize: number) =>
    text.length * fontSize * 1.05;

const PAD_X = 28;
const MIN_BOX_W = 240;

type CardProps = { cx: number; cy: number; label: string; fontSize: number; opacity: number };
const LabelCard: React.FC<CardProps> = ({ cx, cy, label, fontSize, opacity }) => {
    const w = Math.max(MIN_BOX_W, measureLabelWidth(label, fontSize) + PAD_X * 2);
    const h = fontSize * 1.6;
    return (
        <g opacity={opacity}>
            <rect x={cx - w / 2} y={cy - h / 2} width={w} height={h} rx={16} fill={SURFACE} stroke={INK} />
            <text x={cx} y={cy} dominantBaseline="middle" textAnchor="middle"
                  fontFamily="sans-serif" fontSize={fontSize} fill={INK}>
                {label}
            </text>
        </g>
    );
...
複数の箱を整列して並べたい場合（左右対称サブカードなど）は、**全箱の最大幅に揃える**：

```tsx
const labels = ['拡散モデル', '自己回帰モデル'];
const cardW = Math.max(
    MIN_BOX_W,
    ...labels.map(t => measureLabelWidth(t, FS_LABEL) + PAD_X * 2),
);
// 全カードを cardW で描く（短いラベルの箱も同じ幅にしておく）
```

**やってはいけない**:

- 箱を `width={300}` のような固定リテラルで持つ（design_overall の「文字基準」と整合しない）
- テキストを `<text scale={0.6}>` のように縮小して箱に収める（読ませる文字下限 42px を割る）
- テキストを途中で切る／省略記号（…）で隠す（情報欠落）

代替として**箱を持たず `<text>` 単体で配置**する選択肢も常にある（最少構成）。ラベル付き装飾箱を出すなら、必ずテキスト幅から派生計算する。
...
- [ ] 各図が占有ゾーン内に収まり、図どうしが意図せず重ならないか（新規登場図・移動後の着地点を特に確認）
- [ ] **テキストが格納器（箱・カード・帯）からはみ出していないか**。テキスト内包箱は固定 width で持たず、`measureLabelWidth` 等で派生計算しているか（§5.3）
...
- `sandbox/experiments/public_key_crypto/Composition.tsx` — 4段パイプラインで作った完成例。白テーマ、5画面、Track 群フル活用、対話字幕、公開鍵暗号の図解
