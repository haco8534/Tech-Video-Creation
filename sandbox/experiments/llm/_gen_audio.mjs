...
// → プレビューのオーディオが軽い（<Audio> タグ 1 個 / 波形 1 本 / デコード 1 回）。
...
const LLM_DIR = dirname(fileURLToPath(import.meta.url));
const ROOT = resolve(LLM_DIR, '..', '..');
const SRC_DIR = join(LLM_DIR, '_audio_src'); // 個別 WAV（中間生成物）
const PUB_DIR = join(ROOT, 'engine', 'public', 'audio', 'llm');
const VOICE_SRC = 'audio/llm/voice.m4a'; // staticFile 相対
const SPEAKER = { ずんだもん: 3, めたん: 2 }; // VOICEVOX 標準ノーマル

// 音声用テキストの読み差し替え（字幕表示は元の英字のまま。ここは音声だけ）。長い語から先に置換
const READ = [
    ['in-context learning', 'インコンテキストラーニング'],
...
const raw = readFileSync(join(LLM_DIR, 'scriptData.ts'), 'utf8');
...
writeFileSync(join(LLM_DIR, 'audioData.ts'), body, 'utf8');
