...
const SRC_DIR = join(DIR, '_audio_src'); // 個別 WAV（中間生成物）
const PUB_DIR = join(ROOT, 'engine', 'public', 'audio', 'image_generation');
const VOICE_SRC = 'audio/image_generation/voice.m4a'; // staticFile 相対
const SPEAKER = { ずんだもん: 3, めたん: 2 }; // VOICEVOX 標準ノーマル

// 音声用テキストの読み差し替え（字幕表示は元の英字のまま。ここは音声だけ）。長い語から先に置換
const READ = [
    ['a cat on a sofa', 'ア、キャット、オン、ア、ソファ'],
    ['ramen in a bowl', 'ラーメン、イン、ア、ボウル'],
    ['ramen shop', 'ラーメンショップ'],
    ['red shoes', 'レッドシューズ'],
    ['cross-attention', 'クロスアテンション'],
    ['Latent Diffusion', 'レイテントディフュージョン'],
    ['Rectified Flow', 'レクティファイドフロー'],
    ['GPT Image', 'ジーピーティーイメージ'],
    ['OpenAI', 'オープンエーアイ'],
    ['Attention', 'アテンション'],
    ['CLIP', 'クリップ'],
    ['LLM', 'エルエルエム'],
...
const raw = readFileSync(join(DIR, 'scriptData.ts'), 'utf8');
...
writeFileSync(join(DIR, 'audioData.ts'), body, 'utf8');
