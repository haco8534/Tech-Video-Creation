/**
 * ChatGPT Image Auto Generator - Background (Service Worker)
 *
...
const TARGET_URL_PATTERNS = ['chatgpt.com', 'chat.openai.com'];
...
function isChatGPTTab(tab) {
    ...
    return TARGET_URL_PATTERNS.some(p => tab.url.includes(p));
}
...
async function dispatchNext() {
    ...
    try {
        ...
        if (!isChatGPTTab(tab)) {
            await fail(state, 'ChatGPTタブがページから離れました');
            return;
        }
    } catch {
        await fail(state, 'ChatGPTのタブが閉じられました');
        return;
    }
    ...
    try {
        ...
        next.status = 'pending';
        await setQueue(queue);
        await fail(state, 'Content Scriptに接続できません。ChatGPTページをリロードして再試行してください。');
    }
}
...
chrome.runtime.onInstalled.addListener(() => {
    chrome.action.setBadgeText({ text: '' });
    console.log('ChatGPT Image Auto Generator installed.');
});
