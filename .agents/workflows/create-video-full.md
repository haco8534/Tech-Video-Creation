...
- `<canvas>` 要素を使っていないか
- **技術ロゴはすべてdevicon CDNまたはtheSVG（`thesvg.org`）の `<img>` で配置されているか（インラインSVGで再現していないか）**
...
// turbo
8. scene_durations.json を slides/ にコピー（`generate_audio.js` は `{ROOT}/tools/{project_id}/` に出力するが、`generate-subtitle-data.js` は `{SLIDES_DIR}/` を参照するため必須）:
```powershell
...
> レイアウト系プロパティ（width, height, padding, margin, font-size, gap 等）は `projects/creating_programming_language/remotion/scenes/slides.css` を正とし、**一切変更してはならない。**

手順:
- `projects/creating_programming_language/remotion/scenes/slides.css` を `{REMOTION_DIR}/scenes/slides.css` にコピーする
