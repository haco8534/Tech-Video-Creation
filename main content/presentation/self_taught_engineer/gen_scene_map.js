...
const SCRIPT_PATH = path.resolve('d:/myfolder/動画生成/技術解説/台本作成/「独学エンジニア」は本当に通用するのか？/script.md');
const PROJECT_DIR = path.resolve('d:/myfolder/動画生成/技術解説/main content/presentation/self_taught_engineer');
...
const allDialogue = lines.filter(l => /^(ずんだもん|めたん)：/.test(l)).map(l => {
...
scenes.forEach(s => {
  if (s.lines.length === 0 && !s.hold_sec) {
    s.hold_sec = 3;
    console.log('Warning Scene ' + s.id + ' (' + s.title + '): hold_sec=3 auto');
  }
});
