...
### 📢 Remotion側の追加アニメーション（animations.css）

**HTMLプロトタイプ側**では上記 `staggerIn` のみ（プレビュー用）。
**Remotion出力側**では、Phase C1 で基準プロジェクトからコピーされる `animations.css` が自動的に以下を追加する：

| アニメーション | 対象 | 挙動 |
|---|---|---|
| slideInUp | `.content > *` | 下から軽くスライドイン（0.6s one-shot） |
| softRise | `.two-col / .three-col / .flow-chain / .num-list / .bar-chart / .enterprise-grid / .icon-row / .pie-legend` の直下 | 親の出現後、各カードが順番に静かにスライドイン |
| barGrow | `.bar-fill` | `width: 0 → var(--w)` で伸長 |
| metricFade | `.metric-value` | 数値がフェードイン |
| drawPath | `.svg-diagram svg .draw-in`（**opt-in**） | SVGパスのドロー |
| fadeIn | `.fade-in`（**opt-in**） | 単純フェードイン |

**HTML生成時のガイド**:
- SVG矢印・折れ線を「描画するように出現」させたい場合、対象の `<path>` / `<line>` / `<polyline>` に `class="draw-in"` を付与する（プロトタイプでは静止して見えるが、Remotionで動く）
- 特定要素をフェードインだけにしたい場合 `class="fade-in"` を付与
- **既定のスライドイン・バー伸長・カードのポコポコ・メトリクス強調は自動適用**されるため、HTML側で追加クラスは不要
- `animations.css` 自体の改変は禁止（すべての新規動画に波及するため）
