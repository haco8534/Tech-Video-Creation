...
3. **台本作成ワークフロー（`channels/tech_explainer/workflows/script_creation.md`）を参照して実行する。**
   出力先は `{SCRIPT_DIR}/` 配下とする。
...
| 原則 | 詳細 |
|------|------|
| **対等な会話** | めたんは「上から教える先生」ではなく「一緒に考える詳しい友人」。断定的に正解を語るのではなく、ずんだもんの疑問を受け止めて共に探る姿勢で |
| **一般化の注意書き** | 「大きく分けると」「教科書的には」「代表的なものを挙げると」等のヘッジを入れ、これが唯一の正解ではないことを示す |
| **反例・例外への言及** | 二項対立（コンパイラvsインタプリタ、静的vs動的等）を紹介する際は、現実にはグラデーションであることに必ず触れる |
| **誕生エピソードの扱い** | 語り継がれるうちに美化・単純化されている可能性があることをセリフ中で軽く触れる |
| **数値・統計の出典意識** | 数字を出す際は定義や条件で変動しうることに触れる（「〜というデータベースによると」等） |
| **万能解の否定** | どの技術にも長所と短所があり、万能な正解はないことを伝える。特定技術を持ち上げすぎない |
| **趣味と実用の区別** | 「自分でも作れる」と言う際、趣味の小さな言語と実用言語では求められるものが全く違うことを明示する |
...
`{SLIDES_DIR}/design_guide.md` を作成し、カラーパレット・タイポグラフィ・deviconアイコン計画を定義する。SKILL.mdのカラートークン設計に従い、テーマに合わせたCSS変数を決定する。台本に登場する技術名・企業名・サービス名を**すべて**洗い出し、対応するdevicon名（`icons/{name}/{name}-original.svg`）またはtheSVG slug（`icons/{slug}/default.svg`）を一覧化すること。**この一覧に載ったアイコンは B1 で `<img>` または `<image>` タグで必ず配置する。**

### B1. Webプレゼンテーション生成

**SKILL.md（`channels/tech_explainer/skills/presentation_generator/SKILL.md`）を参照して実行する。**
...
// turbo
**アイコン使用検証**（0件ならB2に進まない）:
```powershell
node -e "const fs=require('fs');const html=fs.readFileSync('{SLIDES_DIR}/index.html','utf8');const imgTags=(html.match(/<img\s/g)||[]).length;const imageTags=(html.match(/<image\s/g)||[]).length;const total=imgTags+imageTags;console.log('<img>: '+imgTags+', <image>: '+imageTags+', 合計: '+total);if(total===0){console.log('🚨 アイコンが1つも使われていません！台本中の企業名・技術名にdeviconまたはtheSVGのロゴを配置してください');process.exit(1);}console.log('✅ OK');"
```
...
- `<canvas>` 要素を使っていないか
- **技術ロゴはすべてdevicon CDNまたはtheSVG（`thesvg.org`）の `<img>` または SVG内 `<image>` で配置されているか（インラインSVGで再現していないか）**
...
- **SVG内の人物表現が「頭+肩シルエット」テンプレートに従っているか（`<circle>`1個だけのドットや雑な棒人間は禁止。SKILL.mdの人物アイコンテンプレートを使うこと）**
...
4. **台本中の全英単語をDICTに登録（読み間違いゼロ保証）**:

`{SLIDES_DIR}/scene_map.json` の全セリフから英語テキスト（`[A-Za-z]` を含む単語・フレーズ）を抽出し、`add_readings.js` のDICTに**漏れなく**登録されていることを確認する。

手順:
1. scene_map.json の全 `text` フィールドから `[A-Za-z][A-Za-z0-9.#+\-]*` にマッチする全トークンを抽出
2. 複合語（例: "Crafting Interpreters", "Kotlin Native"）は先に長い方をDICTに登録
3. `add_readings.js` のDICTを読み、未登録の英単語がないか確認
4. 未登録の単語があれば、正しいカタカナ読みをDICTに**追加してからファイルを保存**する
5. DICTは**長いフレーズが先、短い単語が後**の順序を厳守する（部分一致による誤マッチ防止）
6. 追加後、再度全単語がカバーされていることを検証する

> 🚨 **DICT未登録の英単語が1つでもある状態で次のステップに進んではならない。**
> VOICEVOXは未知の英単語をローマ字読みで発音するため、視聴者に違和感を与える。

// turbo
5. 読み付与（scene_map.jsonにreadingフィールド追加）:
```powershell
...
// turbo
8. scene_durations.json を slides/ にコピー（`generate_audio.js` は `{PROJECT_DIR}/tools/` に出力するが、`generate-subtitle-data.js` は `{SLIDES_DIR}/` を参照するため必須）:
```powershell
...
- `{SLIDES_DIR}/scene_durations.json` が `{PROJECT_DIR}/tools/scene_durations.json` と同一である
...
```tsx
import { VideoWithSlides as XxxVideo, TOTAL_FRAMES as XXX_TOTAL_FRAMES } from "@channels/tech_explainer/projects/{project_id}/remotion/VideoWithSlides";
...
2b. `{SCRIPT_DIR}/description.md` を読み込み、`{{TIMESTAMPS}}` プレースホルダーを実タイムスタンプで置換して `{DELIVERABLES_DIR}/概要欄.md` に出力する。

**タイムスタンプ選定ルール（要点に絞る）**:
- `script.md` の **Block境界**（`## 【Block N：タイトル】`）に対応するシーンのみ採用する
- 全シーンを列挙しない。1動画あたり **6〜10個** が目安
- 先頭は必ず `00:00 オープニング`
- Block内の細かいシーン遷移はスキップし、話題が大きく変わるポイントだけ拾う
- タイムスタンプのラベルは視聴者が内容を想像できる短い表現にする（scene_map.jsonのtitleそのままではなく、必要に応じて調整）

例（creating_programming_language の場合）:
```
## 目次
00:00 オープニング
00:37 世界に9,000以上の言語がある
02:33 ソースコードが実行されるまで
05:12 コンパイラ vs インタプリタ
08:22 言語設計の3大決断
13:05 Python・Ruby・Go・Rustの誕生秘話
17:50 自分で言語を作るには
```
